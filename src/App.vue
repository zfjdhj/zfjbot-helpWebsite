<template>
  <van-nav-bar title="猫猫食用手册">
    <template #left>
      <img
        src="zfjbot/help/images/logo.png"
        style="width: 40px; height: 40px"
      />
    </template>
  </van-nav-bar>
  <!-- link 模式，在右侧显示链接箭头 -->
  <van-notice-bar mode="link" left-icon="info-o" @click="noticeBarClick">
    当前适配移动端，点此访问PC端
  </van-notice-bar>
  <van-sticky>
    <form action="/">
  ...
  >
    ...
    <van-collapse v-model="plugins_list['plugins_type_id']" accordion="true">
      <van-collapse-item
        v-for="(plugin, index) in plugins_list['plugins_list']"
        :key="plugin['id']"
        center
        :name="index"
      ...
      >
        ...
        <template #title>
          <div :id="plugin['plugin_name']">
            {{ plugin["plugin_name"] }}
            <van-tag plain type="danger" v-if="plugin['plugin_state']">{{
              plugin["plugin_state"]
            }}</van-tag>
          </div>
        </template>
        ...
        <van-cell-group border="true" style="border: 1px solid pink">
          ...
          <van-cell
            v-for="command in plugin['plugin_commands']"
            :key="command.id"
            center
  ...
  <div v-show="!isRes">
    ...
    <van-cell-group>
      ...
      <van-cell
        v-for="command in res_list.data"
        :key="command.id"
        center
        :value="command.description"
        @click="doCopy(command.command)"
